...
// After a multi-booking transaction is created, block the dates of the
// additional slots on the listing's availability calendar so other users
// can't reserve them. Uses the Integration SDK (operator-level access).
// Failures are logged but do not abort the transaction — the transaction
// already succeeded; this is best-effort blocking.
...
const transactionPromise = (sdk, id) =>
  // listing.author para se saber o modelo de comissao do anfitriao.
  ...
  Promise.all([transactionPromise(sdk, bodyParams?.id), fetchCommission(sdk)])
    .then(responses => {
      ...
      const { providerCommission, customerCommission } = resolveCommission(
        ...
        hostMetadataFrom(showTransactionResponse)
      );
    ...
    .then(apiResponse => {
      ...
      if (!isSpeculative) {
        ...
        const listingId = data?.data?.relationships?.listing?.data?.id;

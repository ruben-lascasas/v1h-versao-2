...
/**
 * GET /api/report-user-status?targetId=<uuid>
 * Returns the current rate-limit status for the authenticated reporter so the
 * UI can show "X/5 today" and warn about same-target/cooldown limits before
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    console.error('[ReportUserStatus] users.show failed:', e?.message || e);
  }

  const now = Date.now();
  const usedToday = history.filter(r => r && now - r.at < DAY_MS).length;
  const alreadyReportedTarget = targetId
    ? history.some(
        r => r && r.targetId === targetId && now - r.at < LIMITS.perTargetDays * DAY_MS
      )
    : false;

...
const server = app.listen(PORT, () => {
  ...
  // Kick off the saved-search email notifier — runs hourly inside this process.
  try {
    require('./jobs/notifySavedSearchesJob').start();
  } catch (e) {
    console.error('[savedSearchNotifier] failed to start:', e?.message || e);
  }
  try {
    require('./jobs/notifyFavoritesJob').start();
  } catch (e) {
    console.error('[favoriteDigest] failed to start:', e?.message || e);
  }
  try {
    require('./jobs/notifyExtraAlertsJob').start();
  } catch (e) {
    console.error('[extraAlerts] failed to start:', e?.message || e);
  }
  try {

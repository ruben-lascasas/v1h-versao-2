...
// Integration SDK credentials (Sharetribe Console > Integrations).
// Used for operator-level actions like creating availability exceptions
// when a multi-booking is created.
...
// Returns the Integration SDK instance if credentials are configured. Used
// for operator-level actions (e.g. creating availability exceptions for
// multi-booking flows). Returns null if creds are missing — callers should
...
exports.getIntegrationSdk = () => {
  ...
  if (cachedIntegrationSdk) return cachedIntegrationSdk;
  try {
    const integrationSdkPkg = require('sharetribe-flex-integration-sdk');
    cachedIntegrationSdk = integrationSdkPkg.createInstance({
    ...
    log.error(e, 'integration-sdk-init-failed');
    return null;
  }
};

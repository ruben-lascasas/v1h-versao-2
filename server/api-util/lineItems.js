...
const getHourQuantityAndLineItems = orderData => {
  ...
  // For multi-booking: sum hours across all additional slots and add to the
  // primary slot's units. The booking entity uses the first slot only; the
  // remaining slots are billed via this combined quantity.
  const additionalSlots = multipleBookings?.additionalBookings || [];
  const additionalUnits = additionalSlots.reduce((sum, s) => {
    const u = calculateQuantityFromHours(s.bookingStart, s.bookingEnd);

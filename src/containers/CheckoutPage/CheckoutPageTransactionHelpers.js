...
export const processCheckoutWithPayment = (orderParams, extraPaymentParams) => {
  ...
  const fnRequestPayment = fnParams => {
    ...
    // Detect long-term booking (>90 days from now). Booking processes only.
    const bookingStartIso =
      fnParams?.bookingStart || fnParams?.bookingDates?.bookingStart;
    const daysUntil = bookingStartIso
      ? (new Date(bookingStartIso).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
      : 0;
    const isLongTermBooking =
      daysUntil > 90 && !!process.transitions.REQUEST_LONG_TERM_RESERVATION;

    // Resume from payment-window-open (timer fired 90 days before event,
    // or operator forced it): customer is paying an existing long-term tx.
    // Also covers the case where pay-long-term-reservation already ran on
    // a previous submit attempt — we must keep treating this as long-term
    // so fnConfirmPayment uses CONFIRM_LONG_TERM_PAYMENT (auto-accept).
    const lastTrName = storedTx?.attributes?.lastTransition;
    const isResumingLongTermPayment =
      lastTrName === process.transitions.OPEN_PAYMENT_WINDOW ||
      lastTrName === process.transitions.OPERATOR_OPEN_PAYMENT_WINDOW ||
      lastTrName === process.transitions.PAY_LONG_TERM_RESERVATION;
  ...
  const fnConfirmPayment = fnParams => {
    ...
    // Long-term payments use a single transition that confirms + accepts + captures,
    // skipping the manual provider-accept step.
    const transitionName = isResumingLongTermPayment
      ? process.transitions.CONFIRM_LONG_TERM_PAYMENT
      : process.transitions.CONFIRM_PAYMENT;
    const isTransitionedAlready =
      storedTx?.attributes?.lastTransition === process.transitions.CONFIRM_PAYMENT ||
      storedTx?.attributes?.lastTransition === process.transitions.CONFIRM_LONG_TERM_PAYMENT;
  ...
  // Detect long-term reservation: bookingStart >90 days from now AND
  // the process has the long-term transition. For long-term we ONLY run
  // fnRequestPayment — the transition has no Stripe action so there's
  // no PaymentIntent, no card-confirm, and no preauthorized state to
  // confirm. The transaction ends in `long-term-reserved`.
  // Exception: if the customer is RESUMING payment for an existing
  // long-term reservation (timer already opened the payment window),
  // we run the full pipeline — pay-long-term-reservation creates a
  // real Stripe PaymentIntent.
  const longTermBookingStart =
    orderParams?.bookingStart || orderParams?.bookingDates?.bookingStart;
  const longTermDaysUntil = longTermBookingStart
    ? (new Date(longTermBookingStart).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
    : 0;
  const lastTrPipeline = storedTx?.attributes?.lastTransition;
  const isResumingLongTermPayment =
    lastTrPipeline === process.transitions.OPEN_PAYMENT_WINDOW ||
    lastTrPipeline === process.transitions.OPERATOR_OPEN_PAYMENT_WINDOW ||
    lastTrPipeline === process.transitions.PAY_LONG_TERM_RESERVATION;
  const isLongTermBooking =
    !isResumingLongTermPayment &&
    longTermDaysUntil > 90 &&
    !!process.transitions.REQUEST_LONG_TERM_RESERVATION;
  ...
  // For long-term we only need fnRequestPayment, but we still need to
  // return the same shape { orderId, paymentMethodSaved } that handleSubmit
  // expects so the redirect-to-OrderDetailsPage works.
  const longTermPipeline = orderParamsX =>
    fnRequestPayment(orderParamsX).then(order => ({
      orderId: order?.id,
      paymentMethodSaved: true,
    }));

  const handlePaymentIntentCreation = isLongTermBooking
    ? longTermPipeline
    : composeAsync(
        fnRequestPayment,

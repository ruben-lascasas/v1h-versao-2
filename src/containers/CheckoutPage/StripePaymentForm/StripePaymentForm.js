...
import { taxId } from '../../../util/validators';
...
const LocationOrShippingDetails = props => {
  ...
  const address = listingLocation?.address;
  const building = listingLocation?.building;
  const noLocationText = intl.formatMessage({ id: 'StripePaymentForm.locationUnknown' });

  const renderAddressWithPin = () => (
    <p className={css.locationDetails}>
      <svg
        ...
        width="16"
        height="16"
        style={{ flexShrink: 0, marginRight: 6, marginTop: 2, fill: '#e53935', verticalAlign: 'text-top' }}
      >
        <path
          fill="#e53935"
...
class StripePaymentForm extends Component {
  ...
  paymentForm(formRenderProps) {
    ...
    const submitDisabled = onetimePaymentNeedsAttention || submitInProgress;
    ...
    return hasStripeKey ? (
      <Form className={classes} onSubmit={handleSubmit} enforcePagePreloadFor="OrderDetailsPage">
            ...
            {showOnetimePaymentFields ? (
              <div className={css.billingDetails}>
                ...
                <FieldTextInput
                  ...
                  inputMode="text"
                  maxLength={20}
                  label={intl.formatMessage({ id: 'StripePaymentForm.taxIdLabel' })}
                  placeholder={intl.formatMessage({ id: 'StripePaymentForm.taxIdPlaceholder' })}
                  validate={taxId(
                    intl.formatMessage({ id: 'StripePaymentForm.taxIdRequired' }),
                    intl.formatMessage({ id: 'StripePaymentForm.taxIdInvalidPt' }),
                    intl.formatMessage({ id: 'StripePaymentForm.taxIdInvalidGeneric' })
                  )}
                />
                ...
                <FieldTextInput
                  ...
                  maxLength={100}
                  label={intl.formatMessage({ id: 'StripePaymentForm.companyNameLabel' })}
                  placeholder={intl.formatMessage({ id: 'StripePaymentForm.companyNamePlaceholder' })}
                />
        ...
        <div className={css.submitContainer}>
          ...
          <PrimaryButton
            className={submitDisabled ? `${css.submitButton} ${css.submitButtonDisabled}` : css.submitButton}
            type="submit"
            inProgress={submitInProgress}
  ...
  render() {
    ...
    return (
      <FinalForm
        ...
        // Without this prop, every Redux state change (e.g. the speculated
        // transaction returning a couple of seconds after the page loads)
        // recreates the initialValues object, which Final Form interprets as
        // "reinitialize" and wipes whatever the user has typed so far.

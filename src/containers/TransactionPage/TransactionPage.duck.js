...
const makeTransitionPayloadCreator = (
  ...
  return makeCall()
    .then(response => {
      ...
      // Response-time tracking for inbox actions (accept/decline/cancel/etc.).
      // Same best-effort pattern as the message-send tracker — failures here
      // must NOT break the transition flow.
      try {
        // eslint-disable-next-line global-require
        const { trackTransitionReplyTime } = require('../../util/responseTime');
        const tx = response?.data?.data || null;
        const currentUser = getState()?.user?.currentUser;
        trackTransitionReplyTime({ sdk, transaction: tx, currentUser });
      } catch (_) {
        // Ignored.
      }
  ...
  return sdk.messages
    ...
    .then(response => {
      ...
      return dispatch(fetchMessagesThunk({ txId, page: 1, config }))
        .then(() => {
          // Best-effort response-time tracking. Looks at the conversation
          // and adds the elapsed gap to the current user's stats. Failures
          // here must NOT break the message-send flow.
          try {
            // eslint-disable-next-line global-require
            ...
            trackReplyTime({ sdk, messages, currentUser });
          } catch (_) {
            // Ignored.
          }
          return messageId;
...
// sendImage    //
//////////////////
// Uploads up to 5 images and sends them as a single message with the marker
// [images:URL1|URL2|...]\n<optional caption>. Mirrors the regular sendMessage
// path so all existing notification / refetch behavior keeps working.
const MAX_TX_IMAGE_BYTES = 10 * 1024 * 1024;
const MAX_TX_IMAGES_PER_BATCH = 6;

const sendImagePayloadCreator = (
  { txId, files, caption, config },
  { dispatch, rejectWithValue, extra: sdk }
) => {
  ...
  if (list.length > MAX_TX_IMAGES_PER_BATCH) {
    return rejectWithValue(storableError(new Error('Too many images')));
  }
  for (const f of list) {
    if (!f.type?.startsWith('image/')) {
      return rejectWithValue(storableError(new Error('Not an image')));
    }
    if (f.size > MAX_TX_IMAGE_BYTES) {
      return rejectWithValue(storableError(new Error('Image too large')));
    }
  }
  ...
  return Promise.all(
    ...
    .then(uploadResps => {
      ...
      const content = trimmedCaption
        ? `📷\n\n${urlBlock}\n\n${trimmedCaption}`
        : `📷\n\n${urlBlock}`;
      return sdk.messages.send({ transactionId: txId, content });
    })
    .then(response => {
      const messageId = response.data.data.id;
      return dispatch(fetchMessagesThunk({ txId, page: 1, config }))
...
export const sendImageThunk = createAsyncThunk(
  'TransactionPage/sendImage',
  sendImagePayloadCreator
);

export const sendImageMessage = (txId, files, caption, config) => dispatch => {
  return dispatch(sendImageThunk({ txId, files, caption, config }));
};

////////////////
// sendReview //
////////////////
// Fire-and-forget: ask the server to recompute publicData.averageRating for the
// reviewed listing so the search-page rating filter can hit Sharetribe directly.
// We don't await or block the review flow on failure.
const triggerRatingRecompute = listingId => {
  if (!listingId) return;
  recomputeListingRating({ listingId }).catch(err => {
    // eslint-disable-next-line no-console
    console.warn('[recomputeListingRating] failed', err);
  });
};
...
const sendReviewPayloadCreator = (
  ...
  const listingId = tx?.relationships?.listing?.data?.id?.uuid || tx?.listing?.id?.uuid;

  // After every review submission we mark a flag so the next time the user
  // lands on the home page, a small site-feedback popup appears asking
  // whether their experience was good and how to improve. Cleared by the
  // popup itself (X or submit). See FeedbackPromptModal.
  const markFeedbackPromptPending = () => {
    if (typeof window === 'undefined') return;
    try {
      window.localStorage.setItem('v1h_pending_feedback_prompt', '1');
  ...
  const sendReviewAsSecond = () => {
    return sdk.transactions
      ...
      .then(response => {
        dispatch(addMarketplaceEntities(response));
        triggerRatingRecompute(listingId);

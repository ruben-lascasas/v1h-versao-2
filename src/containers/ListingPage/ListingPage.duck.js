...
const showListingPayloadCreator = ({ listingId, config, isOwn = false }, thunkAPI) => {
  ...
  // Without this, a transient rate limit during page load surfaces as "Não foi possível carregar o anúncio".
  const fetchWithRetry = (attempt = 0) => {
    const fetcher = isOwn ? sdk.ownListings.show(params) : sdk.listings.show(params);

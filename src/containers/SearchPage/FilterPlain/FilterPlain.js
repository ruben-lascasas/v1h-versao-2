...
class FilterPlainComponent extends Component {
  constructor(props) {
    super(props);
    this.state = { isOpen: true, pendingValues: null, hasPending: false };
  ...
  handleClear(e) {
    ...
    // Reset the Final Form values so the UI (e.g. picked dates) clears even
    // when nothing was confirmed yet (URL params unchanged).
    if (this.formRef.current && this.formRef.current.reset) {
      this.formRef.current.reset({});
    }

    onSubmit(null);
    this.setState({ pendingValues: null, hasPending: false });

...
import { savePreLoginEntriesToSession } from '../../ducks/recentlyViewed.duck';
...
export const SocialLoginButtonsMaybe = props => {
  ...
  const preLoginEntries = useSelector(state => state.recentlyViewed?.entries);

  const authWithFacebook = () => {
    savePreLoginEntriesToSession(preLoginEntries);
    ...
    <div className={css.idpButtons}>
      <div className={css.socialButtonsOr}>
        <span className={css.socialButtonsOrText}>
          {intl.formatMessage({ id: 'AuthenticationPage.or' })}
        </span>
      </div>
      ...
      {showFacebookLogin ? (
        <div className={css.socialButtonWrapper}>
          <SocialLoginButton onClick={() => authWithFacebook()}>
            ...
            <span id="facebook-authentication-msg">{facebookMsg}</span>
          </SocialLoginButton>
        </div>
      ) : null}
      ...
      {showGoogleLogin ? (
        <div className={css.socialButtonWrapper}>
          <SocialLoginButton onClick={() => authWithGoogle()}>
            ...
            <span id="google-authentication-msg">{googleMsg}</span>
          </SocialLoginButton>
        </div>
      ) : null}
    </div>
...
export const AuthenticationForms = props => {
  ...
  const handleSubmitSignup = values => {
    // confirmPassword must never leave the form (it would otherwise fall
    // through getNonUserFieldParams into protectedData as plain text).
    // profissao is handled separately: it's written to operator-only metadata
    // via /api/profile-metadata after signup (see auth.duck.js).
    const {
      userType,
      email,
      password,
      ...
      {isSignupEmailTakenError(signupError) ? (
        <>
          ...
          <NamedLink name="LoginPage">
            <FormattedMessage id="AuthenticationPage.signupFailedEmailAlreadyTakenLoginLink" />
          </NamedLink>
        </>
      ) : (
    ...
    <div className={css.content}>
      <div className={css.formHeader}>
        <Heading as="h1" rootClassName={css.tab}>
          {isLogin
            ? intl.formatMessage({ id: 'AuthenticationPage.loginTitle' })
            : intl.formatMessage({ id: 'AuthenticationPage.signupTitle' })}
        </Heading>
        <p className={css.subtitle}>
          {isLogin
            ? intl.formatMessage({ id: 'AuthenticationPage.loginSubtitle' })
            : intl.formatMessage({ id: 'AuthenticationPage.signupSubtitle' })}
        </p>
      </div>
      {loginOrSignupError}
...
export const AuthenticationPageComponent = props => {
  ...
  // Email verification notice is handled elsewhere (ModalMissingInformation on
  // other pages), so we never render it on the login/signup pages anymore.

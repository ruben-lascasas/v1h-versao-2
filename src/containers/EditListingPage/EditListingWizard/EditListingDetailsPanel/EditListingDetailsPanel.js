      ...
      {canShowEditListingDetailsForm ? (
        <EditListingDetailsForm
          ...
          onSubmit={values => {
            ...
            // Reset the public unique-viewer counter when the listing is being
            // edited in a way that changes what it *represents* (title, type,
            // category, unit). Edits to description, capacity (numeric fields)
            // and amenities (multi-select arrays) keep the counter intact —
            // those are content-level changes, not identity changes.
            try {
              const listingUuid = listing?.id?.uuid;
              if (listingUuid) {
                const prevAttrs = listing.attributes || {};
                const prevPublic = prevAttrs.publicData || {};
                const newPublic = updateValues.publicData || {};

                let shouldReset = false;
                if ((prevAttrs.title || '') !== (updateValues.title || '')) {
                  shouldReset = true;
                }
                if (!shouldReset) {
                  const keys = new Set([
                  ...
                  for (const k of keys) {
                    ...
                    const b = newPublic[k];
                    // Skip "no-reset" field shapes: numbers (capacity-style)
                    // and arrays (amenities-style multi-select).
                ...
                if (shouldReset && typeof fetch === 'function') {
                  ...
                  try {
                    if (typeof window !== 'undefined') {
                      window.localStorage.removeItem(`v1h_viewAllTime_${listingUuid}`);
                      window.localStorage.removeItem(`v1h_viewCount_${listingUuid}`);
                    }
                  } catch (_) {}
                }
              }
            } catch (_) {
              // Counter reset is best-effort; never block the listing save.
            }

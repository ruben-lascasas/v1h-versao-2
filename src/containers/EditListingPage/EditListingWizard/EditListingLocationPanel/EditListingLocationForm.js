...
// Tightened from 60 to 40 — typical content here is short ("Apt 3B",
// "Edifício Aurora, 5º", "Loja 12") and 60 was permissive enough that
// hosts could paste paragraphs of garbage into what's meant to be a
// short address complement.
const BUILDING_MAX_LENGTH = 40;
// Allowed characters: letters (incl. accented), digits, spaces, and common
// address punctuation. Blocks emojis, line breaks, and weird symbols.
const BUILDING_ALLOWED_REGEX = /^[\p{L}\p{N}\s.,#º°ª/\-()]+$/u;
const buildingFormatValid = msg => value =>
  !value || BUILDING_ALLOWED_REGEX.test(value) ? undefined : msg;
        ...
        <Form className={classes} onSubmit={handleSubmit}>
          ...
          <FieldTextInput
            ...
            validate={composeValidators(
              maxLength(
                intl.formatMessage(
                  { id: 'EditListingLocationForm.buildingMaxLength' },
                  { maxLength: BUILDING_MAX_LENGTH }
                ),
                BUILDING_MAX_LENGTH
              ),
              buildingFormatValid(
                intl.formatMessage({ id: 'EditListingLocationForm.buildingInvalidFormat' })
              )
            )}
          />

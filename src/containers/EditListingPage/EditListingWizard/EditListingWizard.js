...
const tabCompleted = (tab, listing, config) => {
  ...
  switch (tab) {
    ...
    case AVAILABILITY:
      // Was `return true` (Sharetribe default), which let the Photos tab
      // unlock even when Localização / Preço / Availability hadn't been
      // touched — the wizard only chains each tab to the previous one's
      // tabCompleted, so a "always true" AVAILABILITY broke the gate.
      // For new listings we require an availabilityPlan to be set; for
      // unbookable processes AVAILABILITY isn't in the tab list at all
      // (see getTabsForListingType) so this branch only runs when the
      // tab is actually being asked about.
...
class EditListingWizard extends Component {
  ...
  render() {
    ...
    if (!invalidExistingListingType && !tabsStatus[selectedTab]) {
      ...
      // Safety: if selectedTab isn't in `tabs` at all (e.g. user typed
      // `/new/photos` directly while the draft only has `TABS_DETAILS_ONLY`),
      // tabs.indexOf returns -1, slice(0, -1) drops the last entry, and we
      // can end up with `undefined` here — which then crashes NamedRedirect
      // with "Missing parameters: tab". Falling back to the first tab is
      // always a valid landing spot.
      const nearestActiveTab =
        tabs
          .slice(0, currentTabIndex >= 0 ? currentTabIndex : tabs.length)
          .reverse()
          .find(t => tabsStatus[t]) || tabs[0];
        ...
        <div className={css.tabsLayout}>
          <h2 className={css.wizardTitle}>
            <FormattedMessage
              id={
                isNewListingFlow
                  ? 'EditListingWizard.createListingTitle'
                  : 'EditListingWizard.editListingTitle'
              }
            />
          </h2>
          <Tabs

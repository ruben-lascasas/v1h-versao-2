    ...
    return onUpdateListingOrCreateListingDraft(tab, updateListingValues)
      .then(r => {
        ...
        if (isNewListingFlow && tab !== AVAILABILITY) {
          ...
          // After saving an existing listing, go back to the listing page
          const to = createResourceLocatorString('ListingPage', routeConfiguration, { id: params.id, slug: params.slug }, {});
          history.push(to);
        }
      })
      .catch(e => {
    ...
    case PRICING_AND_STOCK: {
      return (
        <EditListingPricingAndStockPanel
          ...
          listingMaximumPriceSubUnits={config.listingMaximumPriceSubUnits}
        />
      );
    }
    case PRICING: {

...
const publishListingPayloadCreator = ({ listingId }, { dispatch, getState, rejectWithValue, extra: sdk }) => {
  return sdk.ownListings
    ...
    .then(async response => {
      ...
      // Notifica o admin e aguarda — o servidor fecha o anúncio imediatamente
      // antes de responder, garantindo que fica invisível até aprovação manual.
      // Se falhar não quebra o fluxo, mas o anúncio pode ficar visível brevemente.
      try {
        const published = response?.data?.data;
        const id =
          (typeof listingId === 'string'
            ? listingId
            : listingId?.uuid) || published?.id?.uuid;
        const listingTitle = published?.attributes?.title || null;

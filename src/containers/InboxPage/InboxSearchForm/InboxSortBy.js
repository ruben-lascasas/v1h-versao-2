...
const InboxSortBy = props => {
  ...
  const { rootClassName, className, initialValue, onSelect, intl, excludedKeys = [] } = props;
  ...
  const sortOptions = [
    ...
    { key: 'lastMessageAt', label: intl.formatMessage({ id: 'InboxPage.sortBy.lastMessageAt' }) },
    { key: 'lastTransitionedAt', label: intl.formatMessage({ id: 'InboxPage.sortBy.lastTransitionedAt' }) },
  ].filter(option => !excludedKeys.includes(option.key));
  ...
  return (
    ...
    >
      <MenuLabel id="inbox-sort-by-label" rootClassName={css.sortLabel}>
        <span className={css.sortLabelText}>{menuLabel}</span>
        <IconArrowHead className={iconArrowClassName} direction="down" size="tiny" />
      </MenuLabel>
      <MenuContent className={css.menuContent}>
        {sortOptions.map(option => {
          const selected = initialValue === option.key;
          const itemClass = classNames(css.menuItem, { [css.menuItemSelected]: selected });

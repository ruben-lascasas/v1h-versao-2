...
//////////////////////////
const fetchListingImages = (sdk, listingIds) => {
  ...
  return Promise.all(
    listingIds.map(id =>
      sdk.listings.show({
        ...
        .then(r => ({ id, data: denormalisedResponseEntities(r)?.[0] || null }))
        .catch(() => ({ id, data: null }))
    )
  ).then(results => {
    const map = {};
...
const queryUserReviewsPayloadCreator = ({ userId }, { rejectWithValue, extra: sdk }) => {
  return sdk.reviews.query({
    ...
    .then(reviewsResponse => {
      ...
      // Provider reviews have listing attached; customer reviews need lookup by author (the provider)
      const providerListingIds = [...new Set(
        reviews.map(r => r.listing?.id?.uuid).filter(Boolean)
      )];
      const uniqueProviderIds = [...new Set(
        reviews.filter(r => !r.listing?.id?.uuid).map(r => r.author?.id?.uuid).filter(Boolean)
      )];

      const fetchForProviderReviews = fetchListingImages(sdk, providerListingIds);

      const fetchForCustomerReviews = Promise.all(
        uniqueProviderIds.map(authorId =>
          sdk.listings.query({
            author_id: authorId,
            include: ['images'],
            'fields.image': ['variants.listing-card', 'variants.listing-card-2x'],
            ...
            .then(r => ({ authorId, listing: denormalisedResponseEntities(r)?.[0] || null }))
            .catch(() => ({ authorId, listing: null }))
        )
      );

      return Promise.all([fetchForProviderReviews, fetchForCustomerReviews]).then(
        ([listingDataById, customerListings]) => {
          const authorToListing = {};
          customerListings.forEach(({ authorId, listing }) => {
            if (listing) authorToListing[authorId] = listing;
          });

          return reviews.map(r => {
            const lid = r.listing?.id?.uuid;
            if (lid && listingDataById[lid]) {
              r.listing = listingDataById[lid];
            } else if (!lid) {
              const authorId = r.author?.id?.uuid;
              if (authorId && authorToListing[authorId]) {
                r.listing = authorToListing[authorId];
              }
            }
            return r;
          });

...
export const IncludeScripts = props => {
  ...
  // Gate Google Analytics on the user's analytics-consent choice (GDPR/ePrivacy).
  // Server-side renders skip GA — client picks it up after consent if granted.
  const isClient = typeof window !== 'undefined';
  const analyticsConsentGranted = isClient ? hasConsent('analytics') : false;

  if (googleAnalyticsId && hasGoogleAnalyticsv4Id && analyticsConsentGranted) {
  ...
  if (plausibleDomains && analyticsConsentGranted) {
    // If plausibleDomains is not an empty string, include their script too.
    // Gated by analytics consent for GDPR compliance.
    analyticsLibraries.push(
      <script
        key="plausible"
        defer

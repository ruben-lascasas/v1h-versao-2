...
// and one digit.
export const passwordFormatValid = message => value => {
  if (!value) return VALID;
  const hasLowercase = /[a-zà-öø-ÿ]/.test(value);
  const hasUppercase = /[A-ZÀ-ÖØ-Þ]/.test(value);
  const hasDigit = /[0-9]/.test(value);
  return hasLowercase && hasUppercase && hasDigit ? VALID : message;
};
...
// Portuguese NIF validation. A valid NIF has 9 digits where the 9th is a
// checksum: sum(digit[i] * (9 - i)) for i in 0..7, mod 11. If the remainder
// is 0 or 1 the check digit is 0, else it is 11 - remainder.
export const isValidPortugueseNif = nif => {
  if (typeof nif !== 'string') return false;
  if (!/^\d{9}$/.test(nif)) return false;
  // First digit constraints: NIF can't start with 4 in PT (reserved/unused).
  // Valid first digits: 1, 2 (individuals); 5, 6, 8, 9 (companies/other).
  const firstDigit = parseInt(nif[0], 10);
  const allowedFirst = [1, 2, 3, 5, 6, 8, 9];
  if (!allowedFirst.includes(firstDigit)) return false;
...
// Validator factory: NIF field is always required, but only validated against
// the Portuguese algorithm when country === 'PT'. For other countries we
// accept any 5..20 alphanumeric chars (covers EU VAT IDs, etc.).
export const taxId = (requiredMsg, invalidPtMsg, invalidGenericMsg) => (value, allValues) => {
  if (!value || !String(value).trim()) return requiredMsg;
  const trimmed = String(value).trim().toUpperCase().replace(/\s+/g, '');
  const country = (allValues && allValues.country) || 'PT';
  if (country === 'PT') {
    return isValidPortugueseNif(trimmed) ? VALID : invalidPtMsg;
  }
...
export const moneySubUnitAmountAtMost = (message, maxValue) => value => {
  if (!(value instanceof Money)) return VALID;
  return value.amount <= maxValue ? VALID : message;
};

 ...
 */
/**
 * Returns true if the user has a password set on their account.
 * Uses passwordSet when available, otherwise falls back to identityProviders
 * (a non-empty array signals an IdP-only account with no password).
 */
export const userHasPassword = user => {
  const passwordSet = user?.attributes?.passwordSet;
  if (passwordSet === false) return false;
  if (passwordSet === true) return true;
  const idps = user?.attributes?.identityProviders;
  if (Array.isArray(idps) && idps.length > 0) return false;
  return true;
};

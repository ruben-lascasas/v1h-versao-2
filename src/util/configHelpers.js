...
import { isDemoSharingImage } from './urlHelpers';
...
const mergeLocalizations = (hostedLocalization, defaultLocalization) => {
  // V1H override: prefer the locale from configDefault.js over the hosted asset
  // (Sharetribe Console). This lets us force pt-PT formatting (e.g. "45,00 €")
  // without having to change anything in the Sharetribe Console.
  const locale = defaultLocalization.locale || hostedLocalization?.locale || 'en';
...
const mergeBranding = (brandingConfig, defaultBranding) => {
   ...
   *
   * A da Console continua a mandar — excepto quando é a que veio com a
   * demonstração da Sharetribe. Era esse o caso: em Branding estava a
   * fotografia de um anfiteatro, que não tem nada que ver com a Venue1Hub.
   * Nesse caso vale a nossa, que está no repositório e é a marca.
   */
  const hostedFacebook = getVariantURL(socialSharingImage, 'scaled1200');
  const hostedTwitter = getVariantURL(socialSharingImage, 'scaled600');

  const facebookImage =
    (isDemoSharingImage(hostedFacebook) ? null : hostedFacebook) || defaultBranding.facebookImageURL;
  const twitterImage =
    (isDemoSharingImage(hostedTwitter) ? null : hostedTwitter) || defaultBranding.twitterImageURL;
...
const mergeSearchConfig = (
  ...
  const searchConfig = hostedSearchConfig?.mainSearch
    ? {
        ...
        ratingFilter: defaultSearchConfig.ratingFilter,
        ...hostedSearchConfig,
      }
    : defaultSearchConfig;
  ...
  const ratingFilterMaybe = ratingFilter ? [ratingFilter] : [];

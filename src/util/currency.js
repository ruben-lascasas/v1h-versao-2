...
export const formatMoney = (intl, value) => {
  ...
  const formatted = intl.formatNumber(valueAsNumber, numberFormatOptions);
  // Move currency symbol from start to end: "€ 5,00" → "5,00 €" (also handles "-€ 9,00" → "-9,00 €")
  return formatted.replace(/^(-?)([€$£¥₹])\s*(.+)$/, '$1$3 $2');
};
...
export const formatCurrencyMajorUnit = (intl, currency, valueWithoutSubunits) => {
  ...
  const formatted = intl.formatNumber(valueAsNumber, numberFormatOptions);
  // Move currency symbol from start to end: "€ 5" → "5 €"
  return formatted.replace(/^(-?)([€$£¥₹])\s*(.+)$/, '$1$3 $2');
};

...
class RangeSliderComponent extends Component {
  ...
  render() {
    const { handles, min = 0, max = 10000000, step = 1, ariaLabels = [] } = this.props;
    ...
    return (
      <Track handles={handles} valueToPosition={this.toPosition}>
        {handles.map((h, index) => {
          ...
          // Enforce a step-sized gap between adjacent handles so they never sit
          // on the same value (e.g. min=21, max=21).
          const lowerBound = index === 0 ? min : handles[index - 1] + step;
          const upperBound =
            index === handles.length - 1 ? max : handles[index + 1] - step;
          return (
            <Handle
              key={index}
              className={classes}

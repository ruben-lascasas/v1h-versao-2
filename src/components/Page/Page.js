...
const webmanifestURL = () => {
  // Always use a relative URL so the manifest is served from the same origin
  // as the page. In dev, src/setupProxy.js forwards /site.webmanifest to the
  // dev API server (port 3500). In production the same Express server handles
  // both. Crucially, this makes the manifest reachable when the site is being
  // accessed via ngrok or any tunnel — required for PWA install criteria.
    ...
    return (
      <div id="page" className={classes} style={styles}>
        ...
        >
          ...
          <meta name="apple-mobile-web-app-title" content="V1HUB" />

...
export const getWeekDays = (firstDayOfWeek, intl) => {
  // Some locales (pt-PT in particular) return the full weekday name
  // ("segunda-feira") for `weekday: 'short'` in older Intl implementations,
  // which overflows the calendar header. Truncate to 3 characters and strip
  // any trailing dot to keep the layout consistent across locales.
  const toShort = label => {
    if (!label) return '';
    const noDot = label.replace(/\.$/, '');
    return noDot.slice(0, 3);
  };
  ...
  return new Array(7)
    ...
    .map(day => {
      ...
      const shortLabel = intl.formatDate(date, { weekday: 'short' });
      return [
        toShort(shortLabel),

...
import ShareBudgetButton from '../ShareBudgetButton/ShareBudgetButton';
...
// Fires onFetchTransactionLineItems once on mount when the form is pre-filled
// from a shared budget URL so the price breakdown appears automatically.
const InitialValuesFetcher = ({ values, onFetchLineItems }) => {
  const fired = useRef(false);
  useEffect(() => {
    if (!fired.current && values?.bookingStartTime && values?.bookingEndTime) {
      fired.current = true;
      onFetchLineItems({ values });
    }
  }, []); // only on mount
  return null;
};
...
export const BookingTimeForm = props => {
  ...
  // External initialValues (from shared budget URL) take precedence but keep
  // priceVariantName from internal logic when not explicitly provided.
  const mergedInitialValues = { ...internalInitialValues, ...(externalInitialValues || {}) };
  const initialValuesMaybe = Object.keys(mergedInitialValues).length > 0
      ...
      render={formRenderProps => {
        ...
        return (
          <Form onSubmit={handleSubmit} className={classes} enforcePagePreloadFor="CheckoutPage">
            <InitialValuesFetcher values={values} onFetchLineItems={onHandleFetchLineItems} />

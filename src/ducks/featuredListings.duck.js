...
// Check if section is a listings-type section (supports featured listings)
const isListingsSection = section =>
  section.sectionType === 'listings' || section.sectionType === 'highlightedListing';
...
const fetchFeaturedListingsPayloadCreator = async (arg, thunkAPI) => {
  ...
  const validSelectionTypes = ['newest', 'queryString', 'featured'];
  if (!listingSelection || !validSelectionTypes.includes(listingSelection)) {
    const error = new Error(
      `Invalid listingSelection: "${listingSelection}". Expected "newest", "queryString" or "featured".`
    );
  ...
  return sdk.listings
    .query({
      ...
      'fields.user': ['profile.displayName', 'profile.firstName', 'profile.abbreviatedName', 'profile.avatar'],
      'fields.image': [

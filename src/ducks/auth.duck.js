...
import { clearFavorites } from './favorites.duck';
import { clearRecentlyViewed, clearSessionEntries } from './recentlyViewed.duck';
import { clearFollowing } from './follow.duck';
...
const signupThunk = createAsyncThunk(
  ...
  (params, thunkAPI) => {
    ...
    // "profissao" lives in operator-only metadata (Console user field), which
    // the Marketplace API can't write. Strip it from the create call and send
    // it to our own endpoint (Integration SDK) once the session cookie exists.
    const { profissao, ...createParams } = params;
    ...
    return sdk.currentUser
      ...
      .then(() => {
        if (profissao && typeof fetch !== 'undefined') {
          // Fire-and-forget: a metadata hiccup must not block the signup flow.
        ...
        if (typeof window !== 'undefined') {
          try {
            window.localStorage.setItem('v1h_welcome_pending', '1');
            window.dispatchEvent(new Event('v1h:welcomeFlagSet'));
          } catch (_) {}
        }
        return params;
      })
...
const signupWithIdpThunk = createAsyncThunk(
  ...
  (params, thunkAPI) => {
    ...
    // Como no signup normal: "profissao" vive em metadata (operator-only),
    // por isso sai do create e segue para o nosso endpoint após a sessão.
    const { profissao, ...createParams } = params;
    return createUserWithIdp(createParams)

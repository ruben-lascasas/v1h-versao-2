...
import { getTranslatedConfig } from './util/translateConfig';
...
import CookieConsent, { hasConsent } from './components/CookieConsent/CookieConsent';
...
const Configurations = props => {
  const { appConfig, activeLocale, children } = props;
  ...
  const translatedConfig = getTranslatedConfig(appConfig, activeLocale);

  return (
    <ConfigurationProvider value={translatedConfig}>
...
export const ClientApp = props => {
  ...
  // Locale is a "preferences" cookie under RGPD: only persist if the user
  // consented to that category. Without consent, the choice is in-memory only
  // (resets to default 'pt' on next visit), but switching mid-session still works.
  const [isDark, setIsDark] = useState(() => {
    try {
      return localStorage.getItem('v1h_darkmode') === 'true';
    } catch {
      return false;
    }
  });
  ...
  const toggleDark = () => {
    setIsDark(prev => {
      const next = !prev;
      try { localStorage.setItem('v1h_darkmode', String(next)); } catch {}
      return next;
    });
  };

  // Sync dark mode to body attributes globally so all pages respond.
  useEffect(() => {
    if (isDark) {
      document.body.setAttribute('data-dark-mode', 'true');
      document.body.setAttribute('data-landing-dark', 'true');
    } else {
      document.body.removeAttribute('data-dark-mode');
      document.body.removeAttribute('data-landing-dark');
    }
  }, [isDark]);

  const [locale, setLocaleState] = useState(() => {
    try {
      if (typeof window !== 'undefined' && !hasConsent('preferences')) return 'pt';
      return localStorage.getItem('v1h_locale') || 'pt';
    } catch (e) {
      return 'pt';
    }
  });
  ...
  // If the user revokes preferences consent, drop the saved locale so it isn't
  // surfaced again on next visit (current session can keep the choice).
  useEffect(() => {
    const handler = e => {
      const granted = !!e?.detail?.preferences;
      if (!granted) {
        try { localStorage.removeItem('v1h_locale'); } catch {}
      } else {
        try { localStorage.setItem('v1h_locale', locale); } catch {}
      }
    };
    window.addEventListener('v1h:consentChanged', handler);
    return () => window.removeEventListener('v1h:consentChanged', handler);
  }, [locale]);

  // Global: clicking on selected text clears the selection (avoids "sticky" highlights)
  useEffect(() => {
  ...
  const activeMessages =
    locale === 'pt'
      ? { ...localeMessages, ...hostedTranslations, ...ptMessages }
      : { ...localeMessages };
  ...
  return (
    ...
    <LocaleContext.Provider value={{ locale, setLocale }}>
      <Configurations appConfig={appConfig} activeLocale={locale}>
        <IntlProvider
          locale={appConfig.localization.locale}
          messages={activeMessages}
          textComponent="span"
